...
public class Step4 {
    public static class TraceWatch implements Closeable {
        ...
        public void cost(Runnable run, String taskName) {
            this.start(taskName);
            try {
                run.run();
            } finally {
        ...
        public <T> T cost(Supplier<T> sup, String taskName) {
            this.start(taskName);
            try {
                return sup.get();
            } finally {
    ...
    public void testCost() throws InterruptedException {
        TraceWatch traceWatch = new TraceWatch("测试");
